...
#define GPIO_SET_READ_LOW(x, y)     x->cfgr &= ~(0x3 << (y * 2)); x->omode &= ~(1 << y); x->clr = 1 << y;
#define GPIO_SET_WRITE_LOW(x, y)    x->cfgr &= ~(0x3 << (y * 2)); x->omode &= ~(1 << y); x->cfgr |= 0x1 << (y * 2); x->scr = 1 << y;
